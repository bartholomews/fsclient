...
trait FsClientSttpExtensions {
  ...
  def asUnit: ResponseAs[Either[ResponseError[Nothing], Unit], Nothing] =
  ...
  def mapInto[A, E, B](implicit
    responseMapping: ResponseMapping[A, E, B]
  ): ResponseAs[Either[ResponseError[E], B], Nothing] = responseMapping.responseAs
