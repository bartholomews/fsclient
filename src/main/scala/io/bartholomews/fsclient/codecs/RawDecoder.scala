...
trait RawDecoder[Raw] {
  def decode[F[_]: Sync]: Pipe[F, Response[F], Raw]
}

object RawDecoder {
  val rawJsonPipe: RawDecoder[Json] = new RawDecoder[Json] {
    override def decode[F[_]: Sync]: Pipe[F, Response[F], Json] = _.flatMap(
      _.body
  ...
  val rawPlainTextPipe: RawDecoder[String] = new RawDecoder[String] {
    override def decode[F[_]: Sync]: Pipe[F, Response[F], String] = _.flatMap { response =>
      fs2.Stream
