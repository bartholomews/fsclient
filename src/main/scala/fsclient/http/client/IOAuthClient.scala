...
import fsclient.entities.{AccessToken, HttpRequest, HttpRequestWithBody, OAuthToken}
...
class IOAuthClient(override val consumer: OAuthConsumer, accessToken: AccessToken)(implicit val ec: ExecutionContext)
    extends IOClient(consumer) {

  private val token = Some(OAuthToken(accessToken))

  final def getJson[R](request: HttpRequest.GET[R])(implicit responseDecoder: Decoder[R]): IOResponse[R] =
    super.getJson(request.uri, token)

  final def getPlainText[R](
    request: HttpRequest.GET[R]
  )(implicit responseDecoder: IOHttpPipe[String, R]): IOResponse[R] =
    super.getPlainText(request.uri, token)

  final def fetchJson[B, R](request: HttpRequestWithBody[B, R])(implicit requestBodyEncoder: Encoder[B],
