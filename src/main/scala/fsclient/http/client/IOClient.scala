 ...
 */
class IOClient(val consumer: AppConsumer)(implicit val ec: ExecutionContext) {
  ...
  object auth extends IOBaseClient(consumer) with IOAuthCalls {
    ...
    final override def fetchJson[R](
      ...
      super.fetchJson(request.toHttpRequest[IO](consumer), OAuthEnabled(token))
    ...
    final override def fetchPlainText[R](
      ...
      super.fetchPlainText(request.toHttpRequest[IO](consumer), OAuthEnabled(token))
